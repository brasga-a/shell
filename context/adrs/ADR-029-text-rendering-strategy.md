# ADR-029 — Text Rendering Strategy
...
Use GPUI's text stack while GPUI is the frontend. Do not introduce a parallel text engine prematurely. If a custom renderer is adopted, evaluate dedicated shaping/rasterization stacks such as cosmic-text/swash based on measured requirements.
...
- Avoid duplicate complexity today.
- Keeps custom-renderer path open.
...
- **GPUI text stack**
- **Independent shaping stack now**
- **Dedicated text stack only after custom renderer**
...
- Future renderer migration includes text-stack work.
...
This ADR is validated through: `G18`.
