# ADR-006 — Multi-Monitor State and Surface Ownership
...
Model outputs explicitly. Every per-screen surface and state object must be owned by an OutputId and react to hotplug and per-output scaling.
...
- Correct behavior across multiple displays.
- No hidden single-monitor assumptions.
- Clean lifecycle on hotplug.
...
- More explicit state plumbing.
- Requires careful handling of output races.
...
This ADR is validated through: `G04`.
