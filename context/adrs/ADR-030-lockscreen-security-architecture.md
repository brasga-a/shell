# ADR-030 — Lockscreen Security Architecture
...
Treat lockscreen as a security boundary, not a visual overlay. Production implementation must use the appropriate session-lock protocol and explicitly handle authentication, output hotplug, process failure and input exclusivity.
...
- Prevents shipping a visually convincing but bypassable lockscreen.
...
- Security implementation is independent work and may need PAM or external auth integration.
...
This ADR is validated through: `G21`.
